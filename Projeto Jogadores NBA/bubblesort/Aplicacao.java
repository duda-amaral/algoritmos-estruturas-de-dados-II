...
public class Aplicacao {
	public static void main(String[] args) throws Exception {
		...
		bubbleSort(jogadoresEncontrados, qtdJogadores);
	...
	private static void bubbleSort(Jogador[] jogador, int qtd) {
		...
		do {
			...
			for (int i = qtd - 1; i > 0; i--) {
				for (int j = 0; j < i; j++) {
					if ((jogador[j].getCidadeNascimento().compareTo(jogador[j + 1].getCidadeNascimento()) > 0)
							|| ((jogador[j].getCidadeNascimento().compareTo(jogador[j + 1].getCidadeNascimento()) == 0)
									&& (jogador[j].getNome().compareTo(jogador[j + 1].getNome()) > 0))) {
