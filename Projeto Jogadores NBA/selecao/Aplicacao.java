...
public class Aplicacao {
	public static void main(String[] args) throws Exception {
		...
		selectionSort(jogadoresEncontrados, qtdJogadores);
	...
	public static void selectionSort(Jogador[] jogadores, int n) {
	    for (int i = 0; i < n - 1; i++) {
	        int indiceMinimo = i;
	        
	        for (int j = i + 1; j < n; j++) {
	            if ((jogadores[j].getNome().compareTo(jogadores[indiceMinimo].getNome()) < 0)) {     	
	                indiceMinimo = j;
	            }
	            comparacao++;
	        }
	        
	        Jogador temp = jogadores[indiceMinimo];
	        jogadores[indiceMinimo] = jogadores[i];
	        jogadores[i] = temp;
	        movimentacao++;
	    }
