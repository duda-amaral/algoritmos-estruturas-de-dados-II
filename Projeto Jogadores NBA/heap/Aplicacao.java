...
public class Aplicacao {
	public static void main(String[] args) throws Exception {
		...
		heapSort(jogadoresEncontrados, qtdJogadores);
	...
	public static void heapSort(Jogador[] jogadores, int n) {
	...
	public static void heap(Jogador[] jogadores, int n, int i) {
        int maior = i;
        int esq = 2 * i + 1;
        int dir = 2 * i + 2;

        if (esq < n && compare(jogadores[esq], jogadores[maior]) > 0) {
            maior = esq;
            comparacao++;
        }

        if (dir < n && compare(jogadores[dir], jogadores[maior]) > 0) {
            maior = dir;
            comparacao++;
        }

        if (maior != i) {
            troca(jogadores, i, maior);
            movimentacao++;

            heap(jogadores, n, maior);
        }
    }

    public static int compare(Jogador jogador1, Jogador jogador2) {
        if (jogador1.getAltura() != jogador2.getAltura()) {
            return Integer.compare(jogador1.getAltura(), jogador2.getAltura());
        } else {
            return jogador1.getNome().compareTo(jogador2.getNome());
        }
    }

    public static void troca(Jogador[] jogadores, int i, int j) {
